...
import { generateRandomTime } from "@/utils/gameUtils";
...
export default class GameService {
  ...
  private configuredGame(game: Game): Game {
    switch (game.id) {
      case "numeros-orden":
        ...
        break;
      case "patata-caliente":
        game.config = {
          time: generateRandomTime(20, 40),
        };
        break;
      default:
        break;
  ...
  public async reportPlayerResult(result: number): Promise<void> {
    ...
    await this.firebaseService.write(
      `rooms/${roomCode}/game/results/${playerUuid}`,
      result
    );
  }

  public async resetPlayerScores(roomCode: string): Promise<void> {
    const players: Record<string, Player> = await this.firebaseService.read<
