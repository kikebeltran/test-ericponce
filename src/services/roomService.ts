...
export default class RoomService {
    ...
    public async startGame(roomCode: string, firstGame: boolean = false): Promise<void> {
        if (firstGame) {
            await this.gameService.resetPlayerScores(roomCode);
            this.utilsService.setGamesPlayed(0);
            this.utilsService.setCurrentGameIndex(Math.floor(Math.random() * GameService.GAMES.length));
        }
    ...
    public async updateScores(roomCode: string): Promise<void> {
        const game: Game = await this.firebaseService.read<Game>(`rooms/${roomCode}/game`);
        if (!game) {
            return;
        }
        ...
        for (let i = 0; i < sortedResults.length; i++) {
            ...
            const currentScore = await this.firebaseService.read(`rooms/${roomCode}/players/${uuid}/score`);
